...
<body>
    ...
    <ul class="demand-list">
        ...
        INNER JOIN Sedes ON OS.idSede = Sedes.idSede INNER JOIN StatusOs ON OS.idStatus = StatusOs.idStatus INNER JOIN AssuntoOS ON OS.idAssunto = AssuntoOS.idAssunto WHERE OS.idEmpreiteira = " . $_SESSION['ID_EMPREI'];
            ...
            while ($row = $result->fetch_assoc()){
                ...
                // Evento Modal
                echo '<button class="open-modal" data-ticket-id="' . $row['idOS'] . '">Ver Informações</button>';
                echo '<button class="delete-btn" data-ticket-id="'. $row['idOS']. '">Excluir OS</button>';
                echo '</li>';
